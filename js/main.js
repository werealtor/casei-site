...
function priceFromPricesJSON(map,id,idx0){
  ...
  if(Array.isArray(v)) return v[idx0] ?? null;
  if(v && typeof v==='object'){ const k1=String(idx0+1), k0=String(idx0); return v[k1] ?? v[k0] ?? null; }
  return null;
}
function priceFromConfig(prod,idx0){
  const p=prod?.price;
  if(typeof p==='number') return p;
  if(Array.isArray(p)) return p[idx0] ?? null;
  return null;
}
...
/* ========= 若 main-track 为空，则按 config.json 注入 slides ========= */
async function ensureSlidesAndSeedPrice(){
  ...
  cfg.products.forEach(prod=>{
    ...
    // 为每个 slide 写入 data-price / data-na
...
/* ========= 启动 ========= */
(async function boot(){
  await ensureSlidesAndSeedPrice(); // 若轨道为空则注入 slides，并为每张 slide 写入 data-price
  initSliders();                    // 安装滑块、箭头、进度条与价格联动
})();